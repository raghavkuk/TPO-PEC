...
if($result->num_rows > 0) {
	...
	while($count--) {
		...
		while($row=$result->fetch_assoc()) {
			$rethtml = $rethtml."<tr><td class='data'>".$row['student_id']."</td><td class='data'>".$row['student_name']."</td><td class='data'>".$row['student_programme']."</td><td class='data'>".$row['student_branch']."</td><td class='non-data'><a href='downloadcv.php?filename=".$row['CV URL']."' target='_blank'>Download CV</a></td></tr>";
		}
