...
$rethtml="";
$sql="SELECT dateofvisit,ctc,gross from jaf_details where company_name='$company' AND dateofdept < curdate() AND dateofdept!='0000-00-00' AND reviewed!='0' AND removed='0' AND jobtype='Internship' and programme like '%BE%'";
//echo $sql;
...
if($result->num_rows > 0)
{
		while($row=$result->fetch_assoc()) {
			...
			if($row['ctc']=="" && $row['gross']!="")
				$pkg=$row['gross'];
			$rethtml=$rethtml."<h4><b>Date of Visit:</b> ".$row['dateofvisit']."</h4><h4>Stipend: ".$pkg."</h4>";
		}
}
$sql="SELECT student_branch,gender from placementdetails_beint where company='$company'";
//echo $sql;
$result = $mysqli->query($sql);
if($result->num_rows > 0)
{
	$rethtml=$rethtml."<h4><b>Total Students Hired for Internship:</b> ".$result->num_rows."</h4>";
	$males=0;
	$females=0;
	$aero=0;
	$civil=0;
	...
	while($row=$result->fetch_assoc()) {
		...
		if($row['student_branch']=="Electrical Engineering")
			$ee++;
		if($row['student_branch']=="Electronics and Communication and Engineering")
			$ece++;
	...
	<tr><td>Materials and Metallurgical Engineering</td><td>$mett</td></tr>
